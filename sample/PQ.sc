...
object PQ {
    ...
    PQ(1) match { case PQ(x, xs) => (x, xs.toList) }
                                                  //> res1: (Int, List[Int]) = (1,List())
    PQ(1).insert(2)                               //> res2: scalaz.Heap[Int] = <heap>

    import scalaz._
    import std.option._, std.list._

    PQ(1, 2) match { case PQ(x, xs) => (x, xs.toList) }
                                                  //> res3: (Int, List[Int]) = (1,List(2))

    PQ(1, 2, 3) match { case PQ(x, xs) => (x, xs.toList) }
                                                  //> res4: (Int, List[Int]) = (1,List(2, 3))

    PQ(1, 2, 3, 4) match { case PQ(x, xs) => (x, xs.toList) }
                                                  //> res5: (Int, List[Int]) = (1,List(2, 3, 4))

    PQ(1, 2, 3, 4, 5) match { case PQ(x, xs) => (x, xs.toList) }
                                                  //> res6: (Int, List[Int]) = (1,List(2, 3, 4, 5))

    PQ[Int]                                       //> res7: scalaz.Heap[Int] = <heap>
}
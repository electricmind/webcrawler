...
object TargetVector {
  def apply[F](average: AverageVector[F],
               vs: List[(Double, Vector[F])],
               n: Int)(implicit ord: Ordering[F], cfg: CFG): TargetVector[F] = new TargetVector[F](average, vs, n)

  def apply[F](n: Word = 9)(implicit ord: Ordering[F], cfg: CFG): TargetVector[F] = TargetVector(AverageVector[F](), List[(Priority, Vector[F])](), n)
